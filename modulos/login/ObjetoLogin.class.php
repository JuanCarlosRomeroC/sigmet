 ...
 */
	...
	function  iniciar_sesion($id , $jnombre)
	{
		...
		if (!$rs->EOF){
			...
			$SQL = "SELECT descripcion FROM safor_eje WHERE id_eje='".$_SESSION['seniat_users_eje_sigme']."'";
			$rseje = $DB->Execute($SQL) or die("Error Consultando");
			$_SESSION['seniat_users_eje_des_sigme'] = $rseje->fields['descripcion'];
			$_SESSION['autentificado_sigme'] = "SI";
			$this->ahora = date("Y-n-j H:i:s");
			$_SESSION["ultimoAcceso_sigme"] = $this->ahora;			
			$_SESSION["fingerprint_sigme"] = $this->fingerprint();
			//para que no modifique
			$SQL = "UPDATE seniat_users SET fingerprint ='".$_SESSION["fingerprint_sigme"] ."' WHERE seniat_users_id='".$_SESSION['seniat_users_id_sigme']."'";
			$rs = $DB->Execute($SQL )or die('error db');
			
			$this->inicio_log($id, utf8_encode("Logeado"), $_SESSION['seniat_users_eje_sigme']);
			return "VALIDO";
		}else{
