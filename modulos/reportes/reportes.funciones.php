	...
	{	
		...
		if (!$rs->EOF){
			if ($rs->fields['tipo']=="NOR"){
				...
				$this->js("window.top.location ='r_eje_ae_ai_reg.php?eje=".$eje."'");	
			}
		}*/
		
		$this->js("window.open('r_eje_ae_ai.php?eje=".base64_encode($eje)."')");	
	}
	
	function Ver_Ppto($eje)
	{	
		$this->js("window.open('r_eje_ae_partida.php?eje=".base64_encode($eje)."')");	
