...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(6);
		$this->SetX(100);
		$this->Cell(20,0,'Detalle por Ejecutor A.E y Partida');
	...
	$db=DB_CONECCION();
		...
		if (!$grupo_eje ->EOF)
		{
			...
			$pdf->SetY(9);
			$pdf->SetX(85);
			$pdf->Cell(100,10,$grupo_eje ->fields['descripcion'],0,0,'C');			
			
			$pdf->SetFont('Arial','I',8);
			$SQL = ("SELECT 
						safor_pry_ae_eje_part.id_ae, 
						safor_ae.descripcion
				 	FROM safor_pry_ae_eje_part INNER JOIN safor_ae ON safor_pry_ae_eje_part.id_ae = safor_ae.id_ae
