...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(12);$this->SetX(1);$this->Cell(90,3,'Directriz: Nueva Geopolitica internacional',0,0,'L',1);
		$this->Ln();$this->SetX(1);$this->Cell(100,3,'Objetivo estratégico nacional: Fortalecer la soberanía nacional vigorizando y ampliando las alianzas orientadas a la conformación del bloque geopolitico regional y de un mundo multipolar.',0,0,'L',1);
		$this->Ln();$this->SetX(1);$this->Cell(100,3,'Estrategia: Mantener relaciones soberanas ante el bloque hegemónico mundial.',0,0,'L',1);
		$this->Ln();$this->SetX(1);$this->Cell(100,3,'Objetivo Estratégico Inst: Reordenar el Sistema Aduanero en la concepción Socialista del estado para Promover y Proteger el modelo de desarrollo Socialista.',0,0,'L',1);
		$this->Ln();$this->SetX(1);$this->Cell(100,3,'Enunciado del Objetivo del Proyecto: Fortalecer la capacidad del Servicio Aduanero para la seguridad y defensa nacional, protección y preservación de la vida, el ambiente, la sociedad y la economía nacional',0,0,'L',1);
	...
	$db=DB_CONECCION();
		...
		if (!$grupo_eje ->EOF)
		{
			...
			$pdf->SetY(28);$pdf->SetX(1);$pdf->Cell(40,3,'Gerencia de Aduana Principal:',0,0,'L');
			...
			$SQL = ("SELECT safor_pry_ae_eje_um.id_um, 
				...
				{	
					...
					$pdf->SetX(150);$pdf->Cell(5,10,number_format($grupo_part->fields['may']),0,0,'R');
