...
{	
	...
	{	
		$SQL = "SELECT seniat_users_cerrado_ppto FROM seniat_users_eje WHERE seniat_users_eje='".$eje."' AND seniat_users_id='".$_SESSION['seniat_users_id_sigme']."'"; 
       $db=DB_CONECCION();
       $rs = $db->Execute($SQL ); // resultado sql
       if (!$rs->EOF)
       {
		 $_SESSION['seniat_users_cerrado_ppto_sigme'] = $rs->fields['seniat_users_cerrado_ppto'];
		  
		 if ($_SESSION['seniat_users_cerrado_ppto_sigme']) 
		 {
